 ...
 * @Version 1.0
 */
public class LINKAGE_RULE_DOCTOR_009_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(LINKAGE_RULE_DOCTOR_009_Sink.class);//表名替换

    public LINKAGE_RULE_DOCTOR_009_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("LINKAGE_RULE_DOC_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update linkage_rule_doctor set end_time=? " +    //
                "where LINKAGE_RULE_DOC_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into linkage_rule_doctor " +
                "(LINKAGE_RULE_DOC_ID,LINKAGE_RULE_ID,DOCTOR_ID,EXPERTISE_CODE,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("DOCTOR_ID"));
        insert_ps.setString(4, jsonObject.getString("EXPERTISE_CODE"));
