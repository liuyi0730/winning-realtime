 ...
 * @Version 1.0
 */
public class EXAMINATION_ITEM_CATEGORY_048_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(EXAMINATION_ITEM_CATEGORY_048_Sink.class);//表名替换

    public EXAMINATION_ITEM_CATEGORY_048_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("EXAM_ITEM_CATEGORY_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update examination_item_category set end_time=? " +    //
                "where EXAM_ITEM_CATEGORY_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into examination_item_category " +
                "(EXAM_ITEM_CATEGORY_ID,EXAM_ITEM_CATEGORY_NAME,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,SEQ_NO,BODY_SITE_CODE,ENABLED_FLAG,EXAM_ITEM_CATEGORY_NO,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("EXAM_ITEM_CATEGORY_NAME"));
        insert_ps.setString(3, KeyID_02);
        insert_ps.setString(4, jsonObject.getString("IS_DEL"));
        insert_ps.setTimestamp(5, jsonObject.getTimestamp("CREATED_AT"));
        insert_ps.setTimestamp(6, jsonObject.getTimestamp("MODIFIED_AT"));
        insert_ps.setString(7, jsonObject.getString("SEQ_NO"));
        insert_ps.setString(8, jsonObject.getString("BODY_SITE_CODE"));
        insert_ps.setString(9, jsonObject.getString("ENABLED_FLAG"));
        insert_ps.setString(10, jsonObject.getString("EXAM_ITEM_CATEGORY_NO"));
