 ...
 * @Version 1.0
 */
public class CLINICAL_SERVICE_INSTRUCTION_073_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_SERVICE_INSTRUCTION_073_Sink.class);//表名替换

    public CLINICAL_SERVICE_INSTRUCTION_073_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        String upd_sql = "update clinical_service_instruction set end_time=? " +    //
        ...
        String insert_sql = "insert into clinical_service_instruction " +
                "(ORDER_PERIOD_CODE,CS_ID,ORDER_PERIOD_MODIFIABLE_FLAG,ORDER_ITEM_CNT_MODIFIABLE_FLAG,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(1, jsonObject.getString("ORDER_PERIOD_CODE"));
        insert_ps.setString(2, KeyID_01);
        insert_ps.setString(3, jsonObject.getString("ORDER_PERIOD_MODIFIABLE_FLAG"));
        insert_ps.setString(4, jsonObject.getString("ORDER_ITEM_CNT_MODIFIABLE_FLAG"));
