 ...
 * @Version 1.0
 */
public class CLINICAL_SERVICE_CORRELATION_055_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_SERVICE_CORRELATION_055_Sink.class);//表名替换

    public CLINICAL_SERVICE_CORRELATION_055_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CS_CORRELATION_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_service_correlation set end_time=? " +    //
                "where CS_CORRELATION_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_service_correlation " +
                "(CS_CORRELATION_ID,SOURCE_CS_ID,TARGET_CS_ID,CS_RELATION_TYPE_CODE,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("SOURCE_CS_ID"));
        insert_ps.setString(3, jsonObject.getString("TARGET_CS_ID"));
        insert_ps.setString(4, jsonObject.getString("CS_RELATION_TYPE_CODE"));
