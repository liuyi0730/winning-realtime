 ...
 * @Version 1.0
 */
public class MEDICINE_INSUR_APPROVAL_SCOPE_023_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_INSUR_APPROVAL_SCOPE_023_Sink.class);//表名替换

    public MEDICINE_INSUR_APPROVAL_SCOPE_023_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MED_INSUR_APPROVAL_SCOPE_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_insur_approval_scope set end_time=? " +    //
                "where MED_INSUR_APPROVAL_SCOPE_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_insur_approval_scope " +
                "(MED_INSUR_APPROVAL_SCOPE_ID,MEDICINE_ID,MEDICAL_INSURANCE_CODE,MED_INSTI_INSUR_ID,MED_INSUR_APPROVAL_FLAG,APPROVAL_PROMPT_MESSAGE,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("MEDICAL_INSURANCE_CODE"));
        insert_ps.setString(4, jsonObject.getString("MED_INSTI_INSUR_ID"));
        insert_ps.setString(5, jsonObject.getString("MED_INSUR_APPROVAL_FLAG"));
        insert_ps.setString(6, jsonObject.getString("APPROVAL_PROMPT_MESSAGE"));
        ...
        insert_ps.setTimestamp(11, new Timestamp(jsonObject.getTimestamp("binlog_start_time").getTime()));//改为表的字段数-2
        String op = jsonObject.getString("op");
        if (op.equals("DELETE")) {
            insert_ps.setInt(12, 1);//当有delete操作时:设置is_del为0  //
        } else {
            insert_ps.setInt(12, 0); //默认为0  //
        }
//------------end---添加字段-----------
        if (insert_ps != null) {
