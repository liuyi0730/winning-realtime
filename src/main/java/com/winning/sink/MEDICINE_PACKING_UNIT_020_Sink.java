 ...
 * @Version 1.0
 */
public class MEDICINE_PACKING_UNIT_020_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_PACKING_UNIT_020_Sink.class);//表名替换

    public MEDICINE_PACKING_UNIT_020_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MEDICINE_PACK_UNIT_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_packing_unit set end_time=? " +    //
                "where MEDICINE_PACK_UNIT_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_packing_unit " +
                "(MEDICINE_PACK_UNIT_ID,MEDICINE_ID,REF_PACK_UNIT_CONV_FACTOR,PACK_UNIT_CODE,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,SEQ_NO,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("REF_PACK_UNIT_CONV_FACTOR"));
        insert_ps.setString(4, jsonObject.getString("PACK_UNIT_CODE"));
        ...
        insert_ps.setString(9, jsonObject.getString("SEQ_NO"));
