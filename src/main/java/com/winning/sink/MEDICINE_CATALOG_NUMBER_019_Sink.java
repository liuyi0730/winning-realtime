 ...
 * @Version 1.0
 */
public class MEDICINE_CATALOG_NUMBER_019_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_CATALOG_NUMBER_019_Sink.class);//表名替换

    public MEDICINE_CATALOG_NUMBER_019_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MEDICINE_NO_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_catalog_number set end_time=? " +    //
                "where MEDICINE_NO_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_catalog_number " +
                "(MEDICINE_NO_ID,MEDICINE_ID,MEDICINE_NO,MEDICINE_NO_ENABLE_FLAG,MEMO,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,MEDICINE_NO_TYPE_CODE,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("MEDICINE_NO"));
        insert_ps.setString(4, jsonObject.getString("MEDICINE_NO_ENABLE_FLAG"));
        insert_ps.setString(5, jsonObject.getString("MEMO"));
        ...
        insert_ps.setString(10, jsonObject.getString("MEDICINE_NO_TYPE_CODE"));
