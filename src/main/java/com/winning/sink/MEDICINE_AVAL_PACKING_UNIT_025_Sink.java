 ...
 * @Version 1.0
 */
public class MEDICINE_AVAL_PACKING_UNIT_025_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_AVAL_PACKING_UNIT_025_Sink.class);//表名替换

    public MEDICINE_AVAL_PACKING_UNIT_025_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MEDICINE_AVAL_PACK_UNIT_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_aval_packing_unit set end_time=? " +    //
                "where MEDICINE_AVAL_PACK_UNIT_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_aval_packing_unit " +
                "(MEDICINE_AVAL_PACK_UNIT_ID,MEDICINE_ID,PACK_UNIT_CODE,CONVERT_FACTOR_NUMERATOR,CONVERT_FACTOR_DENOMINATOR,ENABLED_FLAG,SEQ_NO,IS_DEL,HOSPITAL_SOID,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("PACK_UNIT_CODE"));
        insert_ps.setString(4, jsonObject.getString("CONVERT_FACTOR_NUMERATOR"));
        insert_ps.setString(5, jsonObject.getString("CONVERT_FACTOR_DENOMINATOR"));
