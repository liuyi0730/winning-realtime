 ...
 * @Version 1.0
 */
public class ORGANIZATION_CLINICAL_SERVICE_050_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(ORGANIZATION_CLINICAL_SERVICE_050_Sink.class);//表名替换

    public ORGANIZATION_CLINICAL_SERVICE_050_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("ORG_CS_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update organization_clinical_service set end_time=? " +    //
                "where ORG_CS_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into organization_clinical_service " +
                "(ORG_CS_ID,CS_ID,ORG_ID,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,ORG_TYPE_CODE,BU_ROLE_CODE,PARENT_ORG_ID,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("ORG_ID"));
        ...
        insert_ps.setString(8, jsonObject.getString("ORG_TYPE_CODE"));
        insert_ps.setString(9, jsonObject.getString("BU_ROLE_CODE"));
        insert_ps.setString(10, jsonObject.getString("PARENT_ORG_ID"));
