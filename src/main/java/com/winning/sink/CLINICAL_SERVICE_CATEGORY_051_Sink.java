 ...
 * @Version 1.0
 */
public class CLINICAL_SERVICE_CATEGORY_051_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_SERVICE_CATEGORY_051_Sink.class);//表名替换

    public CLINICAL_SERVICE_CATEGORY_051_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CS_CATEGORY_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_service_category set end_time=? " +    //
                "where CS_CATEGORY_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_service_category " +
                "(CS_CATEGORY_ID,CS_CATEGORY_NAME,CS_TYPE_CODE,ORG_ID,CS_CATEGORY_DESC,PINYIN,WUBI,ENABLED_FLAG,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,CS_CATEGORY_NO,PARENT_CS_CATEGORY_ID,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("CS_CATEGORY_NAME"));
        insert_ps.setString(3, jsonObject.getString("CS_TYPE_CODE"));
        insert_ps.setString(4, jsonObject.getString("ORG_ID"));
        insert_ps.setString(5, jsonObject.getString("CS_CATEGORY_DESC"));
        insert_ps.setString(6, jsonObject.getString("PINYIN"));
        insert_ps.setString(7, jsonObject.getString("WUBI"));
        insert_ps.setString(8, jsonObject.getString("ENABLED_FLAG"));
        ...
        insert_ps.setString(13, jsonObject.getString("CS_CATEGORY_NO"));
        insert_ps.setString(14, jsonObject.getString("PARENT_CS_CATEGORY_ID"));
