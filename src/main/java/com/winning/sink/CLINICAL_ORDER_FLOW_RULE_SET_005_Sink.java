 ...
 * @Version 1.0
 */
public class CLINICAL_ORDER_FLOW_RULE_SET_005_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_ORDER_FLOW_RULE_SET_005_Sink.class);//表名替换

    public CLINICAL_ORDER_FLOW_RULE_SET_005_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CLI_ORDER_FLOW_RULE_SET_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_order_flow_rule_set set end_time=? " +    //
                "where CLI_ORDER_FLOW_RULE_SET_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_order_flow_rule_set " +
                "(CLI_ORDER_FLOW_RULE_SET_ID,CLI_ORDER_FLOW_RULE_SET_NAME,CLI_ORDER_FLOW_TYPE_CODE,ENCOUNTER_TYPE_CODE,CS_TYPE_CODE,ORG_ID,ENABLED_FLAG,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("CLI_ORDER_FLOW_RULE_SET_NAME"));
        insert_ps.setString(3, jsonObject.getString("CLI_ORDER_FLOW_TYPE_CODE"));
        insert_ps.setString(4, jsonObject.getString("ENCOUNTER_TYPE_CODE"));
        insert_ps.setString(5, jsonObject.getString("CS_TYPE_CODE"));
        insert_ps.setString(6, jsonObject.getString("ORG_ID"));
        ...
        insert_ps.setTimestamp(12, new Timestamp(jsonObject.getTimestamp("binlog_start_time").getTime()));//改为表的字段数-2
        String op = jsonObject.getString("op");
        if (op.equals("DELETE")) {
            insert_ps.setInt(13, 1);//当有delete操作时:设置is_del为0  //
        } else {
            insert_ps.setInt(13, 0); //默认为0  //
        }
//------------end---添加字段-----------
        if (insert_ps != null) {
