 ...
 * @Version 1.0
 */
public class LINKAGE_TARGET_CS_DETAIL_014_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(LINKAGE_TARGET_CS_DETAIL_014_Sink.class);//表名替换

    public LINKAGE_TARGET_CS_DETAIL_014_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("LINKAGE_TARGET_CS_DETAIL_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update linkage_target_cs_detail set end_time=? " +    //
                "where LINKAGE_TARGET_CS_DETAIL_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into linkage_target_cs_detail " +
                "(LINKAGE_TARGET_CS_DETAIL_ID,LINKAGE_RULE_ID,CS_ID,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,CALCULATION_CARDINAL,LINKAGE_CALC_RULE_CODE,PROMPTED_FLAG,LINKAGE_CALC_DIMENSION_CODE,LINKAGE_CALC_METHOD_CODE,EDITABLE_FLAG,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("LINKAGE_RULE_ID"));
        ...
        insert_ps.setString(8, jsonObject.getString("CALCULATION_CARDINAL"));
        insert_ps.setString(9, jsonObject.getString("LINKAGE_CALC_RULE_CODE"));
        insert_ps.setString(10, jsonObject.getString("PROMPTED_FLAG"));
        insert_ps.setString(11, jsonObject.getString("LINKAGE_CALC_DIMENSION_CODE"));
        insert_ps.setString(12, jsonObject.getString("LINKAGE_CALC_METHOD_CODE"));
        insert_ps.setString(13, jsonObject.getString("EDITABLE_FLAG"));
        insert_ps.setTimestamp(14, new Timestamp(jsonObject.getTimestamp("binlog_start_time").getTime()));//改为表的字段数-2
        String op = jsonObject.getString("op");
        if (op.equals("DELETE")) {
            insert_ps.setInt(15, 1);//当有delete操作时:设置is_del为0  //
        } else {
            insert_ps.setInt(15, 0); //默认为0  //
        }
//------------end---添加字段-----------
        if (insert_ps != null) {
