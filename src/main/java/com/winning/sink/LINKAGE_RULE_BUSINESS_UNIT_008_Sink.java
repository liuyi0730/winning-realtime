 ...
 * @Version 1.0
 */
public class LINKAGE_RULE_BUSINESS_UNIT_008_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(LINKAGE_RULE_BUSINESS_UNIT_008_Sink.class);//表名替换

    public LINKAGE_RULE_BUSINESS_UNIT_008_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("LINKAGE_RULE_BIZ_UNIT_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update linkage_rule_business_unit set end_time=? " +    //
                "where LINKAGE_RULE_BIZ_UNIT_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into linkage_rule_business_unit " +
                "(LINKAGE_RULE_BIZ_UNIT_ID,LINKAGE_RULE_ID,BIZ_UNIT_ID,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("BIZ_UNIT_ID"));
