 ...
 * @Version 1.0
 */
public class MEDICINE_DISTRIBUTED_HOSPITAL_036_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_DISTRIBUTED_HOSPITAL_036_Sink.class);//表名替换

    public MEDICINE_DISTRIBUTED_HOSPITAL_036_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MEDICINE_DISTRIBUTED_HOSP_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_distributed_hospital set end_time=? " +    //
                "where MEDICINE_DISTRIBUTED_HOSP_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_distributed_hospital " +
                "(MEDICINE_DISTRIBUTED_HOSP_ID,MEDICINE_ACTION_ID,MEDICINE_ID,TARGET_HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,HOSPITAL_SOID,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("MEDICINE_ACTION_ID"));
        insert_ps.setString(3, jsonObject.getString("MEDICINE_ID"));
        insert_ps.setString(4, jsonObject.getString("TARGET_HOSPITAL_SOID"));
