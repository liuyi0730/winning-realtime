 ...
 * @Version 1.0
 */
public class LINKAGE_RULE_EXAM_ORDER_016_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(LINKAGE_RULE_EXAM_ORDER_016_Sink.class);//表名替换

    public LINKAGE_RULE_EXAM_ORDER_016_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        String upd_sql = "update linkage_rule_exam_order set end_time=? " +    //
        ...
        String insert_sql = "insert into linkage_rule_exam_order " +
                "(LINKAGE_RULE_ID,LINKAGE_RULE_NAME,LINKAGE_RULE_TYPE_CODE,CONTROL_SCOPE_CODE,CONTROL_INCLUSION_CODE,ENABLED_FLAG,CS_ID,EXAM_ITEM_ID,REDIATION_EXAM_TYPE_CODE,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(7, jsonObject.getString("CS_ID"));
        insert_ps.setString(8, jsonObject.getString("EXAM_ITEM_ID"));
        insert_ps.setString(9, jsonObject.getString("REDIATION_EXAM_TYPE_CODE"));
