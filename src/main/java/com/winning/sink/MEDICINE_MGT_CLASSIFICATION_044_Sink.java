 ...
 * @Version 1.0
 */
public class MEDICINE_MGT_CLASSIFICATION_044_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_MGT_CLASSIFICATION_044_Sink.class);//表名替换

    public MEDICINE_MGT_CLASSIFICATION_044_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MEDICINE_MGT_CLASSIFICATION_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_mgt_classification set end_time=? " +    //
                "where MEDICINE_MGT_CLASSIFICATION_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_mgt_classification " +
                "(MEDICINE_MGT_CLS_NO,MEDICINE_MGT_CLS_NAME,PINYIN,WUBI,PARENT_CLASSIFICATION_ID,SEQ_NO,ENABLED_FLAG,IS_DEL,CREATED_AT,CREATED_BY,MODIFIED_BY,MODIFIED_AT,HOSPITAL_SOID,MEDICINE_MGT_CLASSIFICATION_ID,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(1, jsonObject.getString("MEDICINE_MGT_CLS_NO"));
        insert_ps.setString(2, jsonObject.getString("MEDICINE_MGT_CLS_NAME"));
        insert_ps.setString(3, jsonObject.getString("PINYIN"));
        insert_ps.setString(4, jsonObject.getString("WUBI"));
        insert_ps.setString(5, jsonObject.getString("PARENT_CLASSIFICATION_ID"));
        insert_ps.setString(6, jsonObject.getString("SEQ_NO"));
        ...
        insert_ps.setString(10, jsonObject.getString("CREATED_BY"));
        insert_ps.setString(11, jsonObject.getString("MODIFIED_BY"));
        insert_ps.setTimestamp(12, jsonObject.getTimestamp("MODIFIED_AT"));
        insert_ps.setString(13, KeyID_02);
        insert_ps.setString(14, KeyID_01);
