 ...
 * @Version 1.0
 */
public class MEDICINE_PRICE_TRACE_043_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_PRICE_TRACE_043_Sink.class);//表名替换

    public MEDICINE_PRICE_TRACE_043_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MEDICINE_PRICE_TRACE_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_price_trace set end_time=? " +    //
                "where MEDICINE_PRICE_TRACE_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_price_trace " +
                "(MEDICINE_PRICE_TRACE_ID,MEDICINE_PRICE_ID,MEDICINE_ID,WHOLESALE_PRICE,PURCHASE_PRICE,RETAIL_PRICE,BEGIN_AT,IS_DEL,END_AT,HOSPITAL_SOID,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("MEDICINE_PRICE_ID"));
        insert_ps.setString(3, jsonObject.getString("MEDICINE_ID"));
        insert_ps.setString(4, jsonObject.getString("WHOLESALE_PRICE"));
        insert_ps.setString(5, jsonObject.getString("PURCHASE_PRICE"));
        insert_ps.setString(6, jsonObject.getString("RETAIL_PRICE"));
        insert_ps.setTimestamp(7, jsonObject.getTimestamp("BEGIN_AT"));
        insert_ps.setString(8, jsonObject.getString("IS_DEL"));
        insert_ps.setTimestamp(9, jsonObject.getTimestamp("END_AT"));
