 ...
 * @Version 1.0
 */
public class CLINICAL_SERVICE_NUTRITION_046_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_SERVICE_NUTRITION_046_Sink.class);//表名替换

    public CLINICAL_SERVICE_NUTRITION_046_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        String upd_sql = "update clinical_service_nutrition set end_time=? " +    //
        ...
        String insert_sql = "insert into clinical_service_nutrition " +
                "(CS_ID,NUTRITION_GROUP_CODE,ORDER_PERIOD_CODE,ORDER_PERIOD_MODIFIABLE_FLAG,INTRA_GROUP_EXCLUSIVE_FLAG,INTER_GROUP_EXCLUSIVE_FLAG,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("NUTRITION_GROUP_CODE"));
        ...
        insert_ps.setString(6, jsonObject.getString("INTER_GROUP_EXCLUSIVE_FLAG"));
