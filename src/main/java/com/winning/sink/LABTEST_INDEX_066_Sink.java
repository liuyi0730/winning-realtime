 ...
 * @Version 1.0
 */
public class LABTEST_INDEX_066_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(LABTEST_INDEX_066_Sink.class);//表名替换

    public LABTEST_INDEX_066_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("LABTEST_INDEX_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update labtest_index set end_time=? " +    //
                "where LABTEST_INDEX_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into labtest_index " +
                "(LABTEST_INDEX_ID,LABTEST_SPEC_ID,CS_ID,LABTEST_INDEX_CODE,SINGLE_SELECT_ALLOWED_FLAG,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,DEFAULT_FLAG,SEQ_NO,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("LABTEST_SPEC_ID"));
        insert_ps.setString(3, jsonObject.getString("CS_ID"));
        insert_ps.setString(4, jsonObject.getString("LABTEST_INDEX_CODE"));
        insert_ps.setString(5, jsonObject.getString("SINGLE_SELECT_ALLOWED_FLAG"));
        ...
        insert_ps.setString(10, jsonObject.getString("DEFAULT_FLAG"));
        insert_ps.setString(11, jsonObject.getString("SEQ_NO"));
