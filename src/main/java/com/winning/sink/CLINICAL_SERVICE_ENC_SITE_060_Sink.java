 ...
 * @Version 1.0
 */
public class CLINICAL_SERVICE_ENC_SITE_060_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_SERVICE_ENC_SITE_060_Sink.class);//表名替换

    public CLINICAL_SERVICE_ENC_SITE_060_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CLINICAL_SERVICE_ENC_SITE_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_service_enc_site set end_time=? " +    //
                "where CLINICAL_SERVICE_ENC_SITE_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_service_enc_site " +
                "(CLINICAL_SERVICE_ENC_SITE_ID, CLINICAL_SERVICE_ENC_TYPE_ID, ENCOUNTER_SITE_CODE, HOSPITAL_SOID, IS_DEL, CREATED_AT, MODIFIED_AT, start_time, end_time, physical_delete)" +
                "values (?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("CLINICAL_SERVICE_ENC_TYPE_ID"));
        insert_ps.setString(3, jsonObject.getString("ENCOUNTER_SITE_CODE"));
        ...
        insert_ps.setString(8, jsonObject.getString("BINDING_TIMING_CODE"));
        insert_ps.setTimestamp(9, new Timestamp(jsonObject.getTimestamp("binlog_start_time").getTime() ));//改为表的字段数-2
        String op = jsonObject.getString("op");
        if (op.equals("DELETE")) {
            insert_ps.setInt(10, 1);//当有delete操作时:设置is_del为0  //
        } else {
            insert_ps.setInt(10, 0); //默认为0  //
        }
//------------end---添加字段-----------
        if (insert_ps != null) {
