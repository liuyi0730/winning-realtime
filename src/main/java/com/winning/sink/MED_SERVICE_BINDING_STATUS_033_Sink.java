 ...
 * @Version 1.0
 */
public class MED_SERVICE_BINDING_STATUS_033_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MED_SERVICE_BINDING_STATUS_033_Sink.class);//表名替换

    public MED_SERVICE_BINDING_STATUS_033_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MED_SERVICE_BINDING_STATUS_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update med_service_binding_status set end_time=? " +    //
                "where MED_SERVICE_BINDING_STATUS_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into med_service_binding_status " +
                "(MED_SERVICE_BINDING_STATUS_ID,MEDICINE_ID,CS_ID,MED_SERVICE_BINDING_STATUS,EXECUTED_BY,EXECUTED_AT,DESCRIPTION,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(4, jsonObject.getString("MED_SERVICE_BINDING_STATUS"));
        insert_ps.setString(5, jsonObject.getString("EXECUTED_BY"));
        insert_ps.setTimestamp(6, jsonObject.getTimestamp("EXECUTED_AT"));
        insert_ps.setString(7, jsonObject.getString("DESCRIPTION"));
