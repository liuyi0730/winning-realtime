 ...
 * @Version 1.0
 */
public class CLINICAL_SERVICE_ADT_072_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_SERVICE_ADT_072_Sink.class);//表名替换

    public CLINICAL_SERVICE_ADT_072_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        String upd_sql = "update clinical_service_adt set end_time=? " +    //
        ...
        String insert_sql = "insert into clinical_service_adt " +
                "(ADT_TYPE_CODE,CS_ID,ORDER_PERIOD_CODE,ORDER_PERIOD_MODIFIABLE_FLAG,ORDER_ITEM_CNT_MODIFIABLE_FLAG,ORDER_CANCELLATION_CODE,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,DEFAULT_DISCHARGE_DAYS_OFFSET,DEFAULT_DISCHARGE_TIME,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(1, jsonObject.getString("ADT_TYPE_CODE"));
        insert_ps.setString(2, KeyID_01);
        ...
        insert_ps.setString(5, jsonObject.getString("ORDER_ITEM_CNT_MODIFIABLE_FLAG"));
        insert_ps.setString(6, jsonObject.getString("ORDER_CANCELLATION_CODE"));
        ...
        insert_ps.setString(11, jsonObject.getString("DEFAULT_DISCHARGE_DAYS_OFFSET"));
        insert_ps.setString(12, jsonObject.getString("DEFAULT_DISCHARGE_TIME"));
