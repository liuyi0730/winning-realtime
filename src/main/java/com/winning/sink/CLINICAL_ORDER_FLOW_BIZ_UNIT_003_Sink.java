 ...
 * @Version 1.0
 */
public class CLINICAL_ORDER_FLOW_BIZ_UNIT_003_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_ORDER_FLOW_BIZ_UNIT_003_Sink.class);//表名替换

    public CLINICAL_ORDER_FLOW_BIZ_UNIT_003_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CLI_ORDER_FLOW_BU_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_order_flow_biz_unit set end_time=? " +    //
                "where CLI_ORDER_FLOW_BU_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_order_flow_biz_unit " +
                "(CLI_ORDER_FLOW_BU_ID,CLI_ORDER_FLOW_RULE_ID,BU_ID,EXEC_ORG_ID,PRIORITY,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,SERVICE_START_AT,SERVICE_END_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("CLI_ORDER_FLOW_RULE_ID"));
        insert_ps.setString(3, jsonObject.getString("BU_ID"));
        insert_ps.setString(4, jsonObject.getString("EXEC_ORG_ID"));
        insert_ps.setString(5, jsonObject.getString("PRIORITY"));
        ...
        insert_ps.setTimestamp(10, jsonObject.getTimestamp("SERVICE_START_AT"));
        insert_ps.setTimestamp(11, jsonObject.getTimestamp("SERVICE_END_AT"));
