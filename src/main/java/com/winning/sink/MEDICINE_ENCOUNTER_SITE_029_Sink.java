 ...
 * @Version 1.0
 */
public class MEDICINE_ENCOUNTER_SITE_029_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_ENCOUNTER_SITE_029_Sink.class);//表名替换

    public MEDICINE_ENCOUNTER_SITE_029_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MEDICINE_ENC_SITE_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_encounter_site set end_time=? " +    //
                "where MEDICINE_ENC_SITE_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_encounter_site " +
                "(MEDICINE_ENC_SITE_ID,MEDICINE_ENC_TYPE_ID,ENCOUNTER_SITE_CODE,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
...
//        insert_ps.setString(2, jsonObject.getString("MEDICINE_ENC_SITE_ID"));
        insert_ps.setString(2, jsonObject.getString("MEDICINE_ENC_TYPE_ID"));
