 ...
 * @Version 1.0
 */
public class LABTEST_SPECIFICATION_059_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(LABTEST_SPECIFICATION_059_Sink.class);//表名替换

    public LABTEST_SPECIFICATION_059_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("LABTEST_SPEC_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update labtest_specification set end_time=? " +    //
                "where LABTEST_SPEC_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into labtest_specification " +
                "(LABTEST_SPEC_ID,CS_ID,LABTEST_SPEC_NAME,SHORTCUT,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("LABTEST_SPEC_NAME"));
        insert_ps.setString(4, jsonObject.getString("SHORTCUT"));
