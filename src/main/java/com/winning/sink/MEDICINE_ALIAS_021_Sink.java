 ...
 * @Version 1.0
 */
public class MEDICINE_ALIAS_021_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(MEDICINE_ALIAS_021_Sink.class);//表名替换

    public MEDICINE_ALIAS_021_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("MEDICINE_ALIAS_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update medicine_alias set end_time=? " +    //
                "where MEDICINE_ALIAS_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into medicine_alias " +
                "(MEDICINE_ALIAS_ID,MEDICINE_ID,ALIAS,PINYIN,WUBI,SHORTCUT,MEDICINE_ALIAS_ENABLE_FLAG,MEMO,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("ALIAS"));
        insert_ps.setString(4, jsonObject.getString("PINYIN"));
        insert_ps.setString(5, jsonObject.getString("WUBI"));
        insert_ps.setString(6, jsonObject.getString("SHORTCUT"));
        insert_ps.setString(7, jsonObject.getString("MEDICINE_ALIAS_ENABLE_FLAG"));
        insert_ps.setString(8, jsonObject.getString("MEMO"));
