 ...
 * @Version 1.0
 */
public class CLINICAL_ORDER_FLOW_RULE_GROUP_007_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_ORDER_FLOW_RULE_GROUP_007_Sink.class);//表名替换

    public CLINICAL_ORDER_FLOW_RULE_GROUP_007_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CLI_ORDER_FLOW_RULE_GROUP_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_order_flow_rule_group set end_time=? " +    //
                "where CLI_ORDER_FLOW_RULE_GROUP_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_order_flow_rule_group " +
                "(CLI_ORDER_FLOW_RULE_GROUP_ID,CLI_ORDER_FLOW_RULE_GROUP_NAME,PINYIN,WUBI,HOSPITAL_SOID,IS_DEL,CREATED_BY,CREATED_AT,MODIFIED_BY,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("CLI_ORDER_FLOW_RULE_GROUP_NAME"));
        insert_ps.setString(3, jsonObject.getString("PINYIN"));
        insert_ps.setString(4, jsonObject.getString("WUBI"));
        insert_ps.setString(5, KeyID_02);
        insert_ps.setString(6, jsonObject.getString("IS_DEL"));
        insert_ps.setString(7, jsonObject.getString("CREATED_BY"));
        insert_ps.setTimestamp(8, jsonObject.getTimestamp("CREATED_AT"));
        insert_ps.setString(9, jsonObject.getString("MODIFIED_BY"));
