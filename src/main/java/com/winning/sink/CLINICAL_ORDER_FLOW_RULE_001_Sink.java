 ...
 * @Version 1.0
 */
public class CLINICAL_ORDER_FLOW_RULE_001_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_ORDER_FLOW_RULE_001_Sink.class);//表名替换

    public CLINICAL_ORDER_FLOW_RULE_001_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CLI_ORDER_FLOW_RULE_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_order_flow_rule set end_time=? " +    //
                "where CLI_ORDER_FLOW_RULE_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_order_flow_rule " +
                "(CLI_ORDER_FLOW_RULE_ID,CLI_ORDER_FLOW_RULE_SET_ID,CLI_ORDER_FLOW_RULE_NAME,CLI_ORDER_FLOW_RULE_DESC,PRESCRIBED_BEGIN_AT,PRESCRIBED_END_AT,PRIORITY,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,DEFAULT_FLAG,ENABLED_FLAG,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("CLI_ORDER_FLOW_RULE_SET_ID"));
        insert_ps.setString(3, jsonObject.getString("CLI_ORDER_FLOW_RULE_NAME"));
        insert_ps.setString(4, jsonObject.getString("CLI_ORDER_FLOW_RULE_DESC"));
        insert_ps.setTimestamp(5, jsonObject.getTimestamp("PRESCRIBED_BEGIN_AT"));
        insert_ps.setTimestamp(6, jsonObject.getTimestamp("PRESCRIBED_END_AT"));
        insert_ps.setString(7, jsonObject.getString("PRIORITY"));
        insert_ps.setString(8, KeyID_02);
        ...
        insert_ps.setString(12, jsonObject.getString("DEFAULT_FLAG"));
        insert_ps.setString(13, jsonObject.getString("ENABLED_FLAG"));
