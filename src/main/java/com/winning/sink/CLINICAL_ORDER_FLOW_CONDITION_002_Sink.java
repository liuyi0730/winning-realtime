 ...
 * @Version 1.0
 */
public class CLINICAL_ORDER_FLOW_CONDITION_002_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_ORDER_FLOW_CONDITION_002_Sink.class);//表名替换

    public CLINICAL_ORDER_FLOW_CONDITION_002_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CLI_ORDER_FLOW_CONDITION_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_order_flow_condition set end_time=? " +    //
                "where CLI_ORDER_FLOW_CONDITION_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_order_flow_condition " +
                "(CLI_ORDER_FLOW_CONDITION_ID,CLI_ORDER_FLOW_RULE_ID,CONDITION_DE_SHORT_NAME,CONDITION_VALUE,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,CONDITION_VALUE_NAME,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("CONDITION_DE_SHORT_NAME"));
        insert_ps.setString(4, jsonObject.getString("CONDITION_VALUE"));
        ...
        insert_ps.setString(9, jsonObject.getString("CONDITION_VALUE_NAME"));
