 ...
 * @Version 1.0
 */
public class LINKAGE_RULE_012_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(LINKAGE_RULE_012_Sink.class);//表名替换

    public LINKAGE_RULE_012_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("LINKAGE_RULE_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update linkage_rule set end_time=? " +    //
                "where LINKAGE_RULE_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into linkage_rule " +
                "(LINKAGE_RULE_ID,CS_ID,LINKAGE_RULE_NAME,LINKAGE_RULE_DESC,LINKAGE_RULE_TYPE_CODE,LINKAGE_CALC_RULE_CODE,ENABLED_FLAG,PRIORITY,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,AGE_SEGMENT_CODE,DOSAGE_ROUTE_CODE,SERVED_DECOCTION_CODE,EXPERTISE_CODE,CONTROL_INCLUSION_CODE,CONTROL_SCOPE_CODE,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(3, jsonObject.getString("LINKAGE_RULE_NAME"));
        insert_ps.setString(4, jsonObject.getString("LINKAGE_RULE_DESC"));
        insert_ps.setString(5, jsonObject.getString("LINKAGE_RULE_TYPE_CODE"));
        insert_ps.setString(6, jsonObject.getString("LINKAGE_CALC_RULE_CODE"));
        insert_ps.setString(7, jsonObject.getString("ENABLED_FLAG"));
        insert_ps.setString(8, jsonObject.getString("PRIORITY"));
        ...
        insert_ps.setString(13, jsonObject.getString("AGE_SEGMENT_CODE"));
        insert_ps.setString(14, jsonObject.getString("DOSAGE_ROUTE_CODE"));
        insert_ps.setString(15, jsonObject.getString("SERVED_DECOCTION_CODE"));
        insert_ps.setString(16, jsonObject.getString("EXPERTISE_CODE"));
        insert_ps.setString(17, jsonObject.getString("CONTROL_INCLUSION_CODE"));
        insert_ps.setString(18, jsonObject.getString("CONTROL_SCOPE_CODE"));
        insert_ps.setTimestamp(19, new Timestamp(jsonObject.getTimestamp("binlog_start_time").getTime()));//改为表的字段数-2
        String op = jsonObject.getString("op");
        if (op.equals("DELETE")) {
            insert_ps.setInt(20, 1);//当有delete操作时:设置is_del为0  //
        } else {
            insert_ps.setInt(20, 0); //默认为0  //
        }
//------------end---添加字段-----------
        if (insert_ps != null) {
