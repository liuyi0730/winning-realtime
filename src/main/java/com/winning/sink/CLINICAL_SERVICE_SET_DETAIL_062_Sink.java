 ...
 * @Version 1.0
 */
public class CLINICAL_SERVICE_SET_DETAIL_062_Sink extends TwoPhaseCommitSinkFunction<Tuple1<String>, HikariUtil, Void> { //表名替换
    private static final Logger log = LoggerFactory.getLogger(CLINICAL_SERVICE_SET_DETAIL_062_Sink.class);//表名替换

    public CLINICAL_SERVICE_SET_DETAIL_062_Sink() {//表名替换
    ...
    protected void invoke(HikariUtil HikariUtil, Tuple1<String> tuple, Context context) throws Exception {
        ...
        //主键设置，可能是多个字段组合成主键
		//------------begin---添加字段-----------
        String KeyID_01 = jsonObject.getString("CS_SET_DETAIL_ID"); //
        String KeyID_02 = jsonObject.getString("HOSPITAL_SOID"); //
        String upd_sql = "update clinical_service_set_detail set end_time=? " +    //
                "where CS_SET_DETAIL_ID =? and HOSPITAL_SOID =? and end_time='3000-12-31 00:00:00' "; //
        ...
        String insert_sql = "insert into clinical_service_set_detail " +
                "(CS_SET_DETAIL_ID,CS_SET_ID,CS_ITEM_ID,SEQ_NO,DEFAULT_PRESCRIBED_QTY,DEFAULT_PRESCRIBED_UNIT_CODE,SINGLE_SELECT_ALLOWED_FLAG,DEFAULT_FLAG,MEMO,DESCRIPTION,HOSPITAL_SOID,IS_DEL,CREATED_AT,MODIFIED_AT,start_time,physical_delete) " +
                "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";//改为表的字段总数-1
        ...
        insert_ps.setString(2, jsonObject.getString("CS_SET_ID"));
        insert_ps.setString(3, jsonObject.getString("CS_ITEM_ID"));
        insert_ps.setString(4, jsonObject.getString("SEQ_NO"));
        insert_ps.setString(5, jsonObject.getString("DEFAULT_PRESCRIBED_QTY"));
        insert_ps.setString(6, jsonObject.getString("DEFAULT_PRESCRIBED_UNIT_CODE"));
        insert_ps.setString(7, jsonObject.getString("SINGLE_SELECT_ALLOWED_FLAG"));
        insert_ps.setString(8, jsonObject.getString("DEFAULT_FLAG"));
        insert_ps.setString(9, jsonObject.getString("MEMO"));
        insert_ps.setString(10, jsonObject.getString("DESCRIPTION"));
